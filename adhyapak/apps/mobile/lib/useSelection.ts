 ...
 *
 * Which level is read from `?level=`, matching apps/web/app/prep/useSelection.ts
 * exactly. The web app has a reason of its own for the query string — it is a
 * static export and a dynamic route would need every level prerendered — and
 * the phone keeps it so a link copied from one lands correctly in the other.
 *
 * Falls back to the learner's first level when the parameter is missing or
 * names a level they do not sit, so a stale link opens something real rather
...
export const useSelection = (): {
  ...
  const wanted = useLocalSearchParams<{ level?: string }>().level;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const offersByLevel = await Promise.all(
        mine.map(async (l) => {
          const pick = subjects.find((sub) => sub.levelId === l.id);
      ...
      setSelections(all);
      setSelection(all.find((sel) => sel.level.id === level.id) ?? all[0] ?? null);
      setLoading(false);
    })();
    return () => {
      live = false;
